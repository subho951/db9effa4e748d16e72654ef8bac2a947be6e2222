                        ...
                        <div class="row my-4">
                            <div class="col-md-12 d-flex justify-content-between">
                                ...
                                <?php if(count($fast_buttons) > 3){?>
                                    <a href="<?=url('admin/billing/billing-shortcuts/' . Helper::encoded((($getOrder)?$getOrder->id:'')))?>" class="my-btn btn-sky">MORE</a>
                                <?php }?>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
        ...
        $("#searchForm").submit(function (e) {
            ...
            if(search_keyword != ''){
                if(search_keyword.length >= 3){
                    ...
                    $.ajax({
                        type: "POST",
                        url: base_url + "/admin/billing/search-result",
                        data: formData,
                        cache: false,
                        contentType: false,
                        processData: false,
    ...
    function searchProductCartAdd(itemId, qty, fastButtonId = ''){
        ...
        $.ajax({
            type: "POST",
            url: base_url + "/admin/billing/search-product-add-to-cart",
